...
def push_enabled() -> bool:
    ...
    return os.environ.get("MP_DOCS_PUSH") == "1"


def push_mirror() -> str | None:
    """Отправить зеркало в общий репозиторий. Возвращает текст ошибки или None.

    None — только если отправка прошла: при выключенном пуше тоже вернуть None
    значило бы рапортовать «отправлено», ничего не отправив.
    ...
    """
    if not push_enabled():
        return "пуш выключен (нет MP_DOCS_PUSH=1)"
